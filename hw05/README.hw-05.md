...
* формируем dashboard, показывающий распределение запросов с разными status_code по времени:
  * для `Horizontal axis` выбираем колонку **http_timestamp** из доступных полей
  * для `Vertical axis` устанавливаем функцию **Count of records**
  * для `Break down by` выбираем колонку **http_status_code** из доступных полей
    * если для параметра `Break down by` выбрать функцию **Intervals**, получаем 4 группы статусных кодов
    ![Результат](./hw-05-2.png)<br>
    * если для параметра `Break down by` выбрать функцию **Top Values**, получаем 5 групп статусных кодов (более точно соответствует тому, что в логах)
    ![Результат](./hw-05-1.png)<br>
